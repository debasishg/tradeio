...
object order {
  ...
  object Order {
     ...
     */ 
    private[domain] def create(frontOfficeOrders: List[FrontOfficeOrder]): ErrorOr[List[Order]] = {
      frontOfficeOrders
    ...
    private def createImpl(foOrders: List[FrontOfficeOrder]): List[Order] = {
      foOrders.groupBy(_.accountNo).map { orderForAccount =>
        ...
        val lineItems = fords.map { ford =>
          LineItem(ISINCode(ford.isin), Quantity(ford.qty), UnitPrice(ford.unitPrice), BuySell.withName(ford.buySell))
        }
        Order(
    ...
    private[model] def validateUnitPrice(price: UnitPrice): ValidationResult[BigDecimal] =
      (if (price.value <= 0)
