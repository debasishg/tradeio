...
final class TradeRepositoryInterpreter[M[_]: Sync] private (
      ...
      session.prepare(selectByAccountNoAndDate).use { ps =>
        ps.stream(accountNo ~ date, 1024)
          ...
          .map(_.groupBy(_._2)).map { m =>
            m.map {
              case (refNo, lis) => 
                val singleTradeTaxFeeLine = makeSingleTradeTaxFees(TradeReferenceNo(refNo), lis)
                singleTradeTaxFeeLine.tail.foldLeft(singleTradeTaxFeeLine.head)(Semigroup[Trade].combine)
            }.toList
          }
      }
    }

  private def makeSingleTradeTaxFees(trdRefNo: TradeReferenceNo, 
    trdTxs: List[String ~ 
                 String ~ 
                 String ~ 
                 BuySell ~ 
  ...
  private def storeTradeAndTaxFees(t: Trade, session: Session[M]): M[Trade] = {
    ...
    session.prepareAndExecute(insertTaxFees(t.refNo, t.taxFees.size), t.taxFees).void.map(_ => t)
  }
...
private object TradeQueries {
  ...
  def taxFeeEncoder(refNo: TradeReferenceNo): Encoder[TradeTaxFee] =  
    (varchar ~ taxFeeId ~ numeric).values.contramap((t: TradeTaxFee) => refNo.value ~ t.taxFeeId ~ t.amount.value)
  ...
  def insertTaxFees(tradeRefNo: TradeReferenceNo, n: Int): Command[List[TradeTaxFee]] = {
